...
def trajectories():
    ...
    if Windows:
        file = '/PhD/Experimenten/Jeroen/STRM to images/500ms_watershed/Positions.npz'    
    else:
        file = '/media/timo/Backup Plus/PhD/Experimenten/Jeroen/STRM to images/500ms_watershed/Positions.npz'
    ...
    with np.load(file) as data:
        ...
        N = [np.count_nonzero(k) for k in x]
    
        N_timesteps = np.size(time) # Number of timesteps
        ...
        # All information of the particles will be stored here
        Particles = np.zeros((N_max,N_timesteps,2),dtype=float) # Number of particles X Number of timesteps X coordinates (x,y)
    
        # Store the first particles
        Particles[:,0,0] = x[0,:] # x at time 0
        Particles[:,0,1] = y[0,:] # y at time 0
