...
def distributions():
    ...
    if args.wx and args.wy:
        ...
        for k in range(0,np.size(wx)-1,1):
            TotalArea += wx[k]   * wy[k+1]
            TotalArea -= wx[k+1] * wy[k]
        TotalArea = abs(TotalArea/2)

    # Check which particles lie within the walls, use only those
    thps = 0 # Pseudo-angle
    intersect = np.zeros((N,np.size(wx)-1),dtype=float) # Array to store intersections
    for k in range(0,np.size(wx)-1,1):
        wallx = wx[k:k+2]
        wally = wy[k:k+2]
        
        labda, mu, xinter, yinter = intersectwall(X,Y,wallx[0],wally[0],wallx[1],wally[1],thps)
        
        for i in range(0,N,1):
            if labda[i] > 0 and 0 <= mu[i] <= 1:    #Check for intersection
                intersect[i,k] = 1
    
    intersectionnumber = np.count_nonzero(intersect,axis=1)
    insidedomain = np.where(np.mod(intersectionnumber,2)==1) # Odd number of intersections wanted 

    # Select only the particles inside the domain
    X = X[insidedomain]
    Y = Y[insidedomain]
    N = np.size(X) # Number of particles in domain
    print(N,'particles in domain')
    ...
    print((L-dr/2)/dxr+1,'x dxr  =',format(dxr,'.1e'))
    print((np.pi/2)/dxth,'x dxth =',format(dxth,'.1e'))
